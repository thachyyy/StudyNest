...
import { StorageService, inMemoryStorageObjects } from '../src/services/storage.service.ts';
...
async function runPdfUploadFlowTests() {
  ...
  await test('generateStorageIdentifier creates unique, collision-resistant paths', async () => {
    ...
    assert(id1.startsWith(`topics/${topicId}/documents/`), 'Path includes topics and topic directory');
    assert(id1.endsWith('/source.pdf'), 'Path ends with source.pdf');
    assert(id1 !== id2, 'Successive identifiers are unique due to unique document IDs');
  });
  ...
  await test('Deterministic GCS storage key format generator', async () => {
    const topicId = '11111111-2222-3333-4444-555555555555';
    const documentId = '66666666-7777-8888-9999-000000000000';
    const key = StorageService.getStorageKey(topicId, documentId);
    assert(
      key === `topics/${topicId}/documents/${documentId}/source.pdf`,
      `Key should match deterministic structure, got ${key}`
    );
    const uri = StorageService.getStorageUri(key, 'my-test-bucket');
    assert(
      uri === `gs://my-test-bucket/topics/${topicId}/documents/${documentId}/source.pdf`,
      `URI should match gs:// format, got ${uri}`
    );
  });

  let uploadedDocId: string = '';
  let uploadedStorageUri: string = '';

  await test('Teacher who owns class successfully uploads PDF document to GCS', async () => {
    ...
    assert(
      result.data.sourceUrl?.includes(`topics/${testTopicId}/documents/`) &&
      result.data.sourceUrl?.endsWith('/source.pdf'),
      `sourceUrl contains deterministic GCS storage identifier: ${result.data.sourceUrl}`
    );
    assert(result.data.topicId === testTopicId, 'Attached to correct topic');

    uploadedDocId = result.data.id;
    uploadedStorageUri = result.data.sourceUrl;

    // Verify object exists in storage
    const exists = await StorageService.objectExists(uploadedStorageUri);
    assert(exists, 'Uploaded PDF exists in storage');
  });

  await test('Deleting document cleans up storage object', async () => {
    assert(Boolean(uploadedDocId), 'Document ID from previous upload must exist');
    const deleteResult = await DocumentService.deleteDocument(uploadedDocId, teacherUser);
    assert(deleteResult.status === 200, `Delete status should be 200, got ${deleteResult.status}`);

    const existsAfterDelete = await StorageService.objectExists(uploadedStorageUri);
    assert(!existsAfterDelete, 'Storage object is removed after document deletion');
  });
