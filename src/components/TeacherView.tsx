...
export const TeacherView: React.FC<TeacherViewProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'content' | 'analytics' | 'eval'>('analytics');
  const [selectedMaterial, setSelectedMaterial] = useState<Material>(materials[0]);
  const [selectedStudentForEval, setSelectedStudentForEval] = useState<StudentConversation>(conversations[0]);

  // Form State for uploading / adding new lesson material
  const [newTitle, setNewTitle] = useState('');
  const [newSubject, setNewSubject] = useState('Biology 10');
  const [newClassGroup, setNewClassGroup] = useState('Grade 10A');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-2 shadow-2xs flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 bg-gray-100 p-1 rounded-lg">
          ...
          >
            ...
            <span>Content Management & Text→Tree</span>
          </button>
        ...
        <div className="flex items-center gap-2 pr-2 text-xs text-gray-500">
          ...
          <span>Real-time Learning Tracking</span>
        </div>
      </div>

      {/* SUB-TAB 1: LEARNING ANALYTICS DASHBOARD */}
      {activeSubTab === 'analytics' && (
        <div className="space-y-6">
          {/* Top Metric Cards */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="google-card p-5">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                ...
                <span className="text-xs text-emerald-600 font-semibold flex items-center">
                  <TrendingUp className="w-3 h-3 mr-0.5" /> +8% vs last week
                </span>
              </div>
              <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden mt-3">
                <div className="bg-blue-600 h-full rounded-full" style={{ width: `${analytics.prepCompletionRate}%` }} />
              </div>
            </div>

            <div className="google-card p-5">
              <div className="flex items-center justify-between text-gray-500 text-xs font-medium">
                <span>Class Avg Quiz Score</span>
                <BarChart3 className="w-4 h-4 text-emerald-600" />
              </div>
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-bold text-gray-900">{analytics.avgQuizScore}</span>
                <span className="text-xs text-gray-400">/ 100</span>
              </div>
              <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden mt-3">
                <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${analytics.avgQuizScore}%` }} />
              </div>
            </div>
            ...
            <div className="google-card p-5">
              ...
              <p className="text-xs text-gray-500 mt-3">
                {students.filter(s => s.prepStatus === 'completed').length} completed, {students.filter(s => s.prepStatus === 'in_progress').length} in progress
              </p>
            </div>
      ...
      {/* SUB-TAB 2: CONTENT MANAGEMENT & TEXT TO TREE */}
      {activeSubTab === 'content' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Material Uploader & AI Text -> Tree Form */}
          <div className="lg:col-span-5 google-card p-5">
            <div className="flex items-center gap-2 pb-3 border-b border-gray-200">
              ...
              <div>
                <h3 className="text-base font-bold text-gray-900">Upload & AI Process Material</h3>
                <p className="text-xs text-gray-500">Provide past & upcoming lecture text to auto-generate Knowledge Trees & Keywords</p>
              </div>
            </div>
            ...
            <form onSubmit={handleGenerateAiContent} className="mt-4 space-y-4">
              ...
              <button
                ...
                className="w-full google-btn-primary flex items-center justify-center gap-2 text-xs py-2.5 disabled:opacity-50"
              >
                {isAiProcessing ? (
                  <>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="google-card p-5">
              <div className="flex items-center justify-between pb-3 border-b border-gray-200">
                <h3 className="text-base font-bold text-gray-900">Current Material Library</h3>
                <span className="text-xs text-gray-500">{materials.length} Materials Active</span>
              </div>
              ...
              <>
                ...
                <div className="google-card p-5 bg-gradient-to-r from-blue-50/50 to-indigo-50/30">
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="bg-white p-3 rounded-xl border border-gray-200">
                      <span className="font-bold text-gray-900 block mb-1">Learning Thinking Analysis (Tư duy học tập):</span>
